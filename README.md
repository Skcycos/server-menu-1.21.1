...
构建产物：`build/libs/server_menu-1.0.0.jar`。

## 部署

**服务端**：将 `server_menu-1.0.0.jar` 放入 `mods/`；ApricityUI 为 CLIENT 依赖可省略；业务 Mod 可选，装齐后启动日志会打印：
...
**客户端**：`server_menu-1.0.0.jar` + `ApricityUI-neoforge-1.21.1-1.2.3.1.jar` + 三个业务 Mod 的 JAR 一并放入 mods。
